...
/**
 * ClassicLayout — one-page auto-scroll booking widget.
 *
 * Same data flow & API calls as BookingWidgetDemo, but with:
 * - Numbered section headers with dividers
 * - Inactive sections dimmed (opacity-40, pointer-events-none)
 * - Auto-scroll to active section on step completion
 * - Safe bottom padding (pb-24)
 */
export function ClassicLayout({
  ...
  // Auto-load availability when inputs change (debounced).
  useEffect(() => {
    let cancelled = false;
    const t = setTimeout(() => {
  ...
  useEffect(() => {
    if (slotsReady && !mountedSections.has(2)) {
      setMountedSections(prev => new Set(prev).add(2));
    }
    if (customerReady && !mountedSections.has(3)) {
      setMountedSections(prev => new Set(prev).add(3));
    }
  }, [slotsReady, customerReady, mountedSections]);
  ...
  async function loadAvailability(opts?: { force?: boolean; scroll?: boolean }) {
    ...
    if (cached && now - cached.ts <= 60_000) {
      ...
      setError(null);
      if (!force) { void loadAvailability({ force: true }); return; }
    }
  ...
  /* ── API: create hold (identical logic) ── */
  ...
  /* ── API: pay & confirm (identical logic) ── */
        ...
        <div className="relative space-y-6">
          ...
          {confirmed && (
            <div className="rounded-2xl border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 p-6 text-center animate-in fade-in zoom-in-95 duration-500">
              ...
              <button
                type="button"
                onClick={() => { setConfirmed(false); setConfirmedBookingId(null); void loadAvailability({ force: true }); }}
            ...
            <div className="rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 px-4 py-3 text-sm text-amber-700 dark:text-amber-400 animate-in fade-in slide-in-from-top-2 duration-300" role="alert">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs mt-0.5 opacity-90">Please select a new time below to continue.</div>
                </div>
              </div>
            </div>
          )}
          ...
          <section
            ref={slotsRef}
            className={`transition-all duration-500 ${slotsReady ? mountedSections.has(2) ? 'animate-in fade-in slide-in-from-top-4 duration-500' : '' : 'pointer-events-none opacity-40'}`}
          >
              ...
              <div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {[1, 2, 3, 4, 5, 6].map((n) => (
                  <div key={n} className="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-[#1a1a1a] px-4 py-4 animate-pulse">
                    <div className="h-4 bg-gray-200 dark:bg-white/[0.08] rounded w-24 mb-2"></div>
                    <div className="h-3 bg-gray-100 dark:bg-white/[0.04] rounded w-16"></div>
                  </div>
                ))}
              </div>
            )}
            ...
            {!availLoading && slots.length === 0 && (
              <div className="mt-3 text-sm text-muted-foreground">No slots found for this date. Try another date.</div>
            )}
          </section>
          )}
          ...
          <section
            ref={customerRef}
            className={`transition-all duration-500 ${customerReady ? mountedSections.has(3) ? 'animate-in fade-in slide-in-from-top-4 duration-500' : '' : 'pointer-events-none opacity-40'}`}
          >
    ...
    <div className="flex items-center gap-3">
      <div
        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-colors ${
          active
            ? 'bg-[var(--widget-primary)] text-white'
            : 'bg-gray-200 dark:bg-white/[0.06] text-gray-500 dark:text-white/30'
        }`}
      >
        {number}
      </div>
